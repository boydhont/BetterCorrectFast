...
from .model import visinfo as vi
...
class VisualizationInfoHandler:
    ...
    def _load_visinfo(
        ...
        xml_handler: Optional[AbstractXmlParserSerializer] = None,
    ) -> Optional[vi.VisualizationInfo]:
        if not vp_name:
            return None
        vp_path = topic_dir.joinpath(vp_name)
        if vp_path.exists():
            xml_handler = xml_handler or XmlParserSerializer()
            return xml_handler.parse(vp_path.read_bytes(), vi.VisualizationInfo)
        return None
    ...
    @staticmethod
    def _load_bitmaps(topic_dir: zipfile.Path, visinfo: Optional[vi.VisualizationInfo]) -> dict[str, bytes]:
    ...
    def create_new(
        cls,
        element, # Slightly altered, so it doesn't require ifcopenshell
        xml_handler: Optional[AbstractXmlParserSerializer] = None,
    ...
    def set_selected_elements(self, elements) -> None: # Slightly altered, so it doesn't require ifcopenshell
        visualization_info = self.visualization_info
    ...
    def set_visible_elements(self, elements) -> None: # Slightly altered, so it doesn't require ifcopenshell
        self.set_visibility(elements, elements_visibility="VISIBLE")

    def set_hidden_elements(self, elements) -> None: # Slightly altered, so it doesn't require ifcopenshell
        self.set_visibility(elements, elements_visibility="HIDDEN")

    def set_visibility(
        self, elements, elements_visibility: Literal["VISIBLE", "HIDDEN"] # Slightly altered, so it doesn't require ifcopenshell
    ) -> None:
        visualization_info = self.visualization_info
...
def build_viewpoint(element) -> vi.VisualizationInfo: # Slightly altered, so it doesn't require ifcopenshell
    """
    ...
    unit_scale = ifcopenshell.util.unit.calculate_unit_scale(ifc_file) # TODO make independent of ifcopenshell
    elem_placement = ifcopenshell.util.placement.get_local_placement(element.ObjectPlacement) # TODO make independent of ifcopenshell
    elem_placement[0][3] *= unit_scale
    elem_placement[1][3] *= unit_scale
    elem_placement[2][3] *= unit_scale

    return vi.VisualizationInfo(
        guid=str(uuid.uuid4()),
...
def build_viewpoint_from_position_and_guids(position: NDArray[np.float64], *guids: str) -> vi.VisualizationInfo:
    """
    Return a BCF viewpoint of an IFC element.
...
def build_components(*guids: str) -> vi.Components:
    """
...
def build_camera(elem_placement: NDArray[np.float64]) -> vi.PerspectiveCamera:
    """
